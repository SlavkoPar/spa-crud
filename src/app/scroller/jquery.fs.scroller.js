...
;(function ($, window) {
			...
			return $(this).each(function(i) {
				...
				if (data) {
					...
					if (isActive) {
					    ...
					    $(this).trigger('scroller-hidden');  //(sl)
					} else {
					    data.$scroller.addClass(classes.isActive);
					    $(this).trigger('scroller-shown'); // (sl)
					}
	...
	function onScroll(e) {
		...
		if (data.horizontal) {
			...
			data.$handle.css(handleStyles);  (sl)
        } else {
			// Vertical
			var scrollTop = data.$content.scrollTop();
			...
			data.$handle.css(handleStyles);
			$(this).trigger('on-scroll'); // (sl)
        }

		//data.$handle.css(handleStyles);  (sl) moved above on 2 places
	}
