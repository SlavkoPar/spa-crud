...
var requireJsRuntimeConfig = vm.runInNewContext(fs.readFileSync('src/app/require.config.js') + '; require;'),
    requireJsOptimizerConfig = merge(requireJsRuntimeConfig, {
        ...
        include: [
            ...
            'components/bs-select/BsSelect',
            'components/drop-down/dropdown',
            'components/sl-date-picker/sl-date-picker',
            'components/sl-text-input/sl-text-input',
        ...
        bundles: {
            ...
            'sl-grid': ['components/sl-grid/sl-grid'],
            'sl-pager': ['components/sl-pager/sl-pager'],
            'people': ['components/person-grid/person-grid', 'pages/people/people', 'pages/people/add/person-add', 'pages/people/edit/person-edit']
        }
    }),
    transpilationConfig = {
        root: 'src',
        skip: ['bower_modules/**', 'app/require.config.js', 'components/sl-grid/sl-grid', 'components/sl-pager/sl-pager'],
        babelConfig: {
            modules: 'amd',
            sourceMaps: 'inline'
        }
...
gulp.task('js:optimize', ['js:babel'], function() {
    ...
    return rjs(config)
        .pipe(uglify({ preserveComments: 'some' }))  /* comment while debuging */
        .pipe(gulp.dest('./dist/'));    
})
...
gulp.task('awesome', function () {
    var awesomeCss = gulp.src('./src/bower_modules/font-awesome/css/font-awesome.min.css', { base: './src/bower_modules/font-awesome/' }),
        fontFiles = gulp.src('./src/bower_modules/font-awesome/fonts/*', { base: './src/bower_modules/font-awesome/' });
    return es.concat(awesomeCss, fontFiles)
...
gulp.task('html', function() {
    return gulp.src('./src/index.html')
        .pipe(htmlreplace({
            'css': ['css.css', 'font-awesome/css/font-awesome.min.css'],
            'js': 'scripts.js'
        }))
        .pipe(gulp.dest('./dist/'));
});
...
gulp.task('default', ['html', 'js', 'css'/**/, 'awesome'/**/], function(callback) {
    callback();
